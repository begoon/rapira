...
**Random access — `ПОЗИЦИЯ`**. The spec form `ПОЗИЦИЯ имя = выражение` (§36) sets the file's read/write cursor to a 1-based character offset:

```rapira
ОТКРЫТЬ "data.txt" КАК Ф;
ВВОД ИЗ ФАЙЛА Ф ТЕКСТОВ : ШАПКА;
ПОЗИЦИЯ Ф = 1;                   (* rewind to the start *)
ВВОД ИЗ ФАЙЛА Ф ТЕКСТОВ : СНОВА;
ЗАКРЫТЬ Ф;
```

Position 1 is the first character. Values past EOF clamp to one-past-end (subsequent reads return `''` / `.пусто`). Position 0 or negative is an error.

`ПОЗИЦИЯ_В(имя)` (our addition — not in the original spec) returns the current 1-based position; the argument is the handle's name as text, e.g. `ПОЗИЦИЯ_В("Ф")`.

`ЗАПЕРЕТЬ`, `ОТПЕРЕТЬ`, `СТЕРЕТЬ` are reserved keywords but not yet implemented.
...
- File-state operators `ЗАПЕРЕТЬ`, `ОТПЕРЕТЬ`, `СТЕРЕТЬ`
